...
- `let x : T = E1 in E2` is an annotated let: the binding has type `T` and E1 must have that type (a record literal with matching fields is accepted — see below).
...
- `{x:int = 1, y:bool = false}` is a record literal with an *anonymous* record type `{x:int, y:bool}`; field annotations are optional (`{x = 1}`) and inferred from the value. Fields are read with `p.x`.
...
- A record *literal* can initialize a declared record type where that type is expected (an application argument, an annotated `let`, or a field with that annotation), as long as the fields match by name and type — in any order, reordered to the declaration order. Non-literal values must have the exact same type:
  ...
  - `let q = {x:int = 1, y:bool = false} in (\p : point . p.x) q` ✗ (`q` has the anonymous type)
