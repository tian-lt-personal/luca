...
struct lex_err_eof {
  friend std::strong_ordering operator<=>(const lex_err_eof&, const lex_err_eof&) = default;
};
struct lex_err_unknown {
  friend std::strong_ordering operator<=>(const lex_err_unknown&, const lex_err_unknown&) = default;
};
