...
TEST(machine_tests, higher_order) { EXPECT_EQ(std::get<int>(eval_ok("(\\f : int . f 5) (\\x : int . x + 1)")), 6); }
...
TEST(machine_tests, let_with_function) {
  // sema can't deduce arrow types through vars yet, so use let only for the arg
  EXPECT_EQ(std::get<int>(eval_ok("(\\f : int . f 5) (\\x : int . x + 1)")), 6);
}

TEST(machine_tests, let_passed_through) {
  // (\f:int. (\g:int. g 2) (f 1)) (\x:int. \y:int. x + y)
  EXPECT_EQ(std::get<int>(eval_ok("(\\f : int . (\\g : int . g 2) (f 1)) (\\x : int . \\y : int . x + y)")), 3);
}
